...
export const docs = {
  ...
  props: [
    ...
    },
    {
      ...
      description: 'Enables collapse behavior. true for uncontrolled with default toggle button, or an object for controlled mode and advanced config (defaultIsCollapsed, isCollapsed + onCollapsedChange, hasButton, buttonLabel). A controlled config can also be passed to a SideNavCollapseButton rendered outside this SideNav, so both share one state. collapsedWidth sets the collapsed width in px. It defaults to the icon rail, and 0 hides the nav entirely: pair that with an outside SideNavCollapseButton, since the built-in one hides with the nav. A fully hidden nav is made inert, and if focus is inside it when the collapse starts, focus is parked on that outside toggle (or blurred) before inert lands, so it is never silently yanked. isAnimated slides the content out and back in for collapsedWidth: 0. Only transform animates; the box itself snaps in a single reflow, and the icon rail always snaps.',
      default: 'false',
    },
    {
      name: 'resizable',
